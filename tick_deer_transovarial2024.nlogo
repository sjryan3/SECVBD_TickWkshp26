breed [ lonestars lonestar]      ;; red turtles = lone star ticks
breed [ deer a_deer ]   ;; green turtles = white tailed deer
breed [ hosts host ]   ;; yellow turtle = all other hosts


lonestars-own [
  life_stage   ;; life_stage of this tick 0 = egg, 1 = larva, 2 = nymph, 3 = adult
  current_activity  ;; activity of tick: 0 = resting, 1 = questing, 2 = feeding, 3 = give birth and die (females only)
  time_in_current_activity ;; timer for current activity stage
  my_hosts ;; list to hold species and id of each successful bloodmeal
  sex ;; 0 = male, 1 = female
]

deer-own [
  ticks_on_me ;; list to hold ticks currently on this deer
  ticks_ever_on_me ;; number to hold the total ticks this deer has ever seen
]

hosts-own [
  ticks_on_me ;; list to hold ticks currently on this host
  ticks_ever_on_me ;; number to hold the total ticks this deer has ever seen
]

turtles-own [
  disease_status ;; current infection status of individual, 0 = susceptible, 1 = infected, 2 = immune
]

globals [
  prevalence_lonestar ;; keeps track of disease prevalence
  prevalence_deer ;; keeps track of disease prevalence
  prevalence_hosts ;; keeps track of disease prevalence
  max_ticks_deer  ;; maximum number of ticks on deer
  max_ticks_hosts ;; maximum number of ticks on hosts
  successful_attach  ;; chance of successfully attaching to a host or deer
  ;;lonestar_mortality
  deer_mortality
  host_mortality
  lonestar_born
  deer_born
  host_born
  time_check
  day_of_year ;; translation of ticks to day of year
  year_num ;; number of years simulation has run to set next emergence time
]

to-report random-near [center]  ;; turtle procedure
  let result 0
  repeat 40
    [ set result (result + random center) ]
  report result / 20
end

to startup
  ;; (for this model to work with NetLogo's new plotting features,
  ;; __clear-all-and-reset-ticks should be replaced with clear-all at
  ;; the beginning of your setup procedure and reset-ticks at the end
  ;; of the procedure.)
  __clear-all-and-reset-ticks
  setup-prev-plot
  setup-pop-plot
end

;; observer procedure to set up model
to setup
  reset-ticks
  ;; (for this model to work with NetLogo's new plotting features,
  ;; __clear-all-and-reset-ticks should be replaced with clear-all at
  ;; the beginning of your setup procedure and reset-ticks at the end
  ;; of the procedure.)
  __clear-all-and-reset-ticks                  ;; clears view -- don't use ca so MM plot doesn't clear
  setup-prev-plot
  ;set-current-plot "Prevalences"
  ;clear-plot
  setup-pop-plot
  ;set-current-plot "Populations"
  setup-tick-plot
  set year_num 1
  set prevalence_lonestar 0
  set prevalence_deer 0
  set prevalence_hosts 0
  ;;set lonestar_mortality
  set deer_mortality 0.002
  set host_mortality 0.02
  set deer_born 0
  set host_born 0
  set lonestar_born 0
  set max_ticks_deer 50 ;; maximum number of ticks on deer
  set max_ticks_hosts 200 ;; maximum number of ticks on hosts
  set successful_attach 0.75 ;; chance of successfully attaching to a host or deer
  create-lonestars 5000 ;; add larvae based on slider
  [
   ifelse random 100 < initial_prev
     [set disease_status 1]
     [set disease_status 0]
   set sex random 2
   ifelse disease_status = 0
      [set color green]
      [set color red]
;;   setxy random-xcor random-ycor ;; randomly anywhere on grid
   setxy random-float max-pxcor random-float max-pycor ;; only in first quadrant
   ;;setxy 1 1
   set life_stage 1 ;;(random 2 + 1)  ;; life_stage of this tick ;; note all adults are dead at this time
   set current_activity 0 ;; activity of tick: 0 = resting, 1 = questing, 2 = feeding
   ;; Assume we start January 1
   ifelse life_stage = 1
       [set time_in_current_activity 150] ;; resting larva mature to questing nymphs
       [set time_in_current_activity 150] ;; resting nymphs mature to questing adults
   set my_hosts (list) ;;
   set shape "spider"
   set hidden? TRUE ;;(current_activity = 0 or current_activity = 2) ;; only show ticks questing
  ]
  create-lonestars 250 ;; add nymphs based on slider
  [
   ifelse random 100 < initial_prev
     [set disease_status 1]
     [set disease_status 0]
   set sex random 2
   ifelse disease_status = 0
      [set color green]
      [set color red]
   setxy random-xcor random-ycor
   ;;setxy 1 1
   set life_stage 2 ;;(random 2 + 1)  ;; life_stage of this tick ;; note all adults are dead at this time
   set current_activity 0 ;; activity of tick: 0 = resting, 1 = questing, 2 = feeding
   ;; Assume we start January 1
   ifelse life_stage = 1
       [set time_in_current_activity random 45 + 240] ;; resting larva mature to questing nymphs
       [set time_in_current_activity random 75 + 180] ;; resting nymphs mature to questing adults
   set my_hosts (list) ;;
   set shape "spider"
   set hidden? TRUE ;;(current_activity = 0 or current_activity = 2) ;; only show ticks questing
  ]
  create-lonestars 10000 ;; add eggs
  [
   ifelse random 100 < initial_prev
     [set disease_status 1]
     [set disease_status 0]
    set sex random 2
    set color green
    setxy random-xcor random-ycor
    set life_stage 0  ;; eggs
    set current_activity 0 ;; activity of tick: 0 = resting, 1 = questing, 2 = feeding
   ;; Assume we start January 1
   set time_in_current_activity 210
   set my_hosts (list) ;;
   set shape "spider"
   set hidden? TRUE ;;(current_activity = 0 or current_activity = 2) ;; only show ticks questing
  ]

  create-deer 20  ;; add hosts based on slider
  [
    set disease_status 0 ;;random 2
    setxy random-xcor random-ycor
    ifelse disease_status = 0
      [set color green]
      [set color red]
    set shape "sheep"
    set ticks_on_me (list)
    set ticks_ever_on_me 0
  ]

  create-hosts 500  ;; add hosts based on slider
  [
    set disease_status 0 ;;random 2
    setxy random-xcor random-ycor
    ifelse disease_status = 0
      [set color green]
      [set color red]
    set shape "rabbit"
    set ticks_on_me (list)
    set ticks_ever_on_me 0
  ]
end

;; main procedure
to go
  tick
  set day_of_year ticks mod 360
  if day_of_year = 0 [set year_num year_num + 1]
  ;;set time_check  (random 75 + (year_num + 1) * 360 + 180 - ticks)
  ask lonestars [ any_lonestars_die ]    ;; check for mortality from environmental causes
  ask deer [ any_deer_die ]
  ask hosts [ any_hosts_die ]
  ask deer [ deer_move ]                ;; hosts can move
  ask hosts [ hosts_move ]                ;; hosts can move
  ask lonestars [if current_activity = 1 [questing ] ]
  ask lonestars [ change_activity ]         ;; ticks check life stage to see if they change activities based on time
  ask deer [ disease_deer ]     ;; calculate everyone's chance of changing disease status based on interactions
  ask hosts [ disease_hosts ]     ;; calculate everyone's chance of changing disease status based on interactions
  ask lonestars [ disease_lonestars ]     ;; calculate everyone's chance of changing disease status based on interactions
  plot-prevalences                 ;; plots prevalence
  plot-population                ;; plots all population levels
  plot-ticks-only              ;; plots tick questing population, etc.
end

to any_lonestars_die
  let current_ticks count lonestars
  let max_lonestars 50000
  ;; need to make mortality based on time of year... currently a monthly variable, eventually tied to weather
  let my_month floor ( day_of_year / 30 )
  let monthly_lonestar_mortality [0.1 0.1 0.1 0.01 0.01 0.01 0.01 0.01 0.01 0.1 0.1 0.1]
  let my_stage_mortality (item my_month monthly_lonestar_mortality) * lonestar_mortality * current_ticks / max_lonestars;; * ( 5 - life_stage)
  if random-float 1.0 < my_stage_mortality
  [
    if current_activity = 2
    [
      ask last my_hosts [set ticks_on_me remove myself ticks_on_me]
    ]
    die
  ]
end

to any_deer_die
  if random-float 1.0 < deer_mortality
  [
    set deer_born (deer_born + 1)
    hatch-deer 1 ;; immediately replace deer
    [
      set disease_status 0
      setxy random-xcor random-ycor
      set ticks_on_me (list)
      set ticks_ever_on_me 0
      set color green
    ]
    ;; assume ticks feeding die too
    foreach ticks_on_me [ [?1] -> ask ?1
      [ die
      ]
    ]
    die
  ]
end

to any_hosts_die
  if random-float 1.0 < host_mortality
  [
    set host_born (host_born + 1)
    hatch-hosts 1 ;; immediately replace deer
    [
      set disease_status 0
      setxy random-xcor random-ycor
      set ticks_on_me (list)
      set ticks_ever_on_me 0
      set color green
    ]
    ;; assume ticks feeding die too
    foreach ticks_on_me [ [?1] -> ask ?1
      [ die
      ]
    ]
    die
  ]
end

to deer_move  ;; deer movement procedure
  fd 1
  rt random-float 360
end

to hosts_move  ;; hosts movement procedure can be different distance or direction
  fd 1
  rt random-float 360
end

to questing ;; ticks look for deer or host to jump onto
;; look for deer first
;; attach to deer
  let potential_deer one-of (deer-here) with [length ticks_on_me < max_ticks_deer]
    ifelse potential_deer != nobody and random-float 1.0 < successful_attach
    [
      set my_hosts lput potential_deer my_hosts
      ask potential_deer [ set ticks_on_me lput myself ticks_on_me ]
      ask potential_deer [ set ticks_ever_on_me (ticks_ever_on_me + 1)]
      set time_in_current_activity 3 ;; three days to complete blood meal
      set current_activity 2 ;; change to feeding
    ]
    [
      let potential_host one-of (hosts-here) with [length ticks_on_me < max_ticks_hosts]
      if potential_host != nobody and random-float 1.0 < successful_attach
      [
        set my_hosts lput potential_host my_hosts
        ask potential_host [ set ticks_on_me lput myself ticks_on_me]
        ask potential_host [ set ticks_ever_on_me (ticks_ever_on_me + 1)]
        set time_in_current_activity 3 ;; three days to complete blood meal
        set current_activity 2 ;; change to feeding
      ]
    ]

end

to change_activity
  ifelse time_in_current_activity = 0
  [ ;; move to next stage
    ifelse current_activity = 0
    [   ;;0 -> 1  resting -> questing
      set current_activity 1
      set time_in_current_activity 42 ;; giving them 42 days to find a host
      set life_stage (life_stage + 1) ;; change life stage here to keep simple as eggs don't quest!
    ]
    [
      ifelse current_activity = 1
      [     ;;1 questing failed if you reach here
        die
      ]
      [     ;; current activity = 2 and got here means successfully fed, ready to molt
        ;; need to remove self from deer or host list
        let host_in_question last my_hosts
        ask host_in_question [set ticks_on_me remove myself ticks_on_me]
        setxy ([xcor] of host_in_question) ([ycor] of host_in_question) ;; fall off where my last host is today

        ifelse life_stage < 3
        [ ;; not adult
          set current_activity 0 ;; put into resting mode again
          let questing_used time_in_current_activity ;; subtract time used for questing from time to molt
          if life_stage = 1
            [
              set time_in_current_activity  (random 75 + year_num * 360 + 180 - ticks)
              ] ;; molt takes 9 months
          if life_stage = 2
            [
              set time_in_current_activity (random 90 + year_num * 360 + 150 - ticks)
              ]
        ]
        [ ;; adults
          ifelse sex = 0
          [ ;; males die at end of meal
            die
          ]
          [ ;; females give birth and die
            hatch-lonestars Eggs ;; add hosts based on slider
            [
              ifelse disease_status = 0
              [ set disease_status 0]
              [ ifelse random 100 < transovarial_prev
                [set disease_status 1]
                [set disease_status 0]]
              set sex random 2
              ifelse disease_status = 0
                [set color red]
                [set color green]
              set life_stage 0  ;; eggs
              set current_activity 0 ;; activity of tick: 0 = resting, 1 = questing, 2 = feeding
              set time_in_current_activity (random 45 + year_num * 360 + 240 - ticks) ;;350 ;; timer for current activity stage
              set my_hosts [ ] ;;
              set shape "spider"
              set hidden? TRUE ;;(current_activity = 0 or current_activity = 2) ;; only show ticks questing
             ]
             die
          ]
        ]
      ]
    ]
  ]
  [ ;; decrease time_in_current_activity
    set time_in_current_activity (time_in_current_activity - 1) ;; decrease timer
  ]
end

to disease_deer
 ifelse disease_status = 0
    [

      let my_infected_ticks 0
      foreach ticks_on_me [ [?1] ->
        ask ?1
        [
        if disease_status = 1
          [
            set my_infected_ticks (my_infected_ticks + 1)
          ]
        ]
      ]
      if my_infected_ticks > 0
      [
      ;; check to see if could be infected during that time frame
        if prob_tick_infect_deer * my_infected_ticks > random-float 1.0
        [
          set disease_status 1
          set color red
        ]
      ]
    ]
    [
      if disease_status = 1 and prob_deer_recover > random-float 1.0
      [
        set disease_status 0
        set color green
      ]
    ]
end

to disease_hosts
 ifelse disease_status = 0
    [

      let my_infected_ticks 0
      foreach ticks_on_me [ [?1] ->
        ask ?1
        [
        if disease_status = 1
          [
            set my_infected_ticks (my_infected_ticks + 1)
          ]
        ]
      ]
      if my_infected_ticks > 0
      [
      ;; check to see if could be infected during that time frame
        if prob_tick_infect_host * my_infected_ticks > random-float 1.0
        [
          set disease_status 1
          set color red
        ]
      ]
    ]
    [
      if disease_status = 1 and prob_host_recover > random-float 1.0
      [
        set disease_status 0
        set color green
      ]
    ]
 end

to disease_lonestars
  if current_activity = 2 ;; only chance in infection status while feeding
  [  if disease_status = 0 ;; no super_infection
     [
     let host_in_question last my_hosts
     ifelse member? host_in_question deer
     [
     if ([disease_status] of host_in_question = 1) and (prob_deer_infect_tick  > random-float 1.0)
        [
          set disease_status 1
        ]
      ;; check to see if could be infected during that time frame
     ]
     [
       if ([disease_status] of host_in_question = 1) and (prob_host_infect_tick  > random-float 1.0)
        [
          set disease_status 1
        ]
     ]
     ]
  ]
end

to setup-prev-plot
  set-current-plot "Prevalences"
  clear-plot
end

to setup-pop-plot
  set-current-plot "Populations"
  clear-plot
end

to setup-tick-plot
  set-current-plot "Ticks_Only"
  clear-plot
end


to plot-prevalences
  set-current-plot "Prevalences"
  set-current-plot-pen "lonestars"
  ifelse count lonestars with [current_activity = 1 and life_stage > 1] != 0
  [
    plot (count lonestars with [disease_status = 1 and current_activity = 1 and life_stage > 1])/(count lonestars with [current_activity = 1 and life_stage > 1])
  ]
  [
    plot 0
  ]
  set-current-plot-pen "deer"
  ifelse count deer != 0
  [
    plot (count deer with [disease_status = 1])/(count deer)
  ]
  [
    plot 0
  ]
  set-current-plot-pen "hosts"
  ifelse count hosts != 0
  [
    plot (count hosts with [disease_status = 1])/(count hosts)
  ]
  [
    plot 0
  ]
end

to plot-population
  set-current-plot "Populations"
  set-current-plot-pen "lonestars"
  plot (count lonestars)
  set-current-plot-pen "ticks_no_egg"
  plot (count lonestars with [life_stage > 0])
  set-current-plot-pen "deer"
  plot (count deer)
  set-current-plot-pen "hosts"
  plot (count hosts)
end

to plot-ticks-only
  set-current-plot "Ticks_Only"
  set-current-plot-pen "questing"
  plot (count lonestars with [current_activity = 1])
  set-current-plot-pen "adult"
  plot (count lonestars with [life_stage = 3])
  set-current-plot-pen "nymph"
  plot (count lonestars with [life_stage = 2])
  set-current-plot-pen "larvae"
  plot (count lonestars with [life_stage = 1])
  ;;set-current-plot-pen "eggs"
  ;;plot (count lonestars with [life_stage = 0])


  ;;set-current-plot-pen "deer"
  ;plot (count deer)
  ;set-current-plot-pen "hosts"
  ;plot (count hosts)
end

;; An enzyme forms a complex by colliding on a patch with a substrate
;; or inhibitor.  If it collides with an inhibitor, it always forms
;; a complex.  If it collides with a substrate, Kc is its percent chance
;; of forming a complex.
;to form-complex  ;; enzyme procedure
;  if partner != nobody [ stop ]
 ; set partner one-of (other turtles-here with [partner = nobody])
 ; if partner = nobody [ stop ]
;  if [partner] of partner != nobody [ set partner nobody stop ]  ;; just in case two enzymes grab the same partner
;  ifelse ((([breed] of partner) = substrates) and ((random-float 100) < Kc))
;     or (([breed] of partner) = inhibitors)
;    [ ask partner [ set partner myself ]
;      setshape
;      ask partner [ setshape ] ]
;;    [ set partner nobody ]
;end

;; substrate procedure that controls the rate at which complexed substrates
;; are converted into products and released from the complex
;to react-forward
;  if (partner != nobody) and (random-float 1000 < Kr)
;    [ set breed products
;      ask partner [ set partner nobody ]
;      let old-partner partner
;      set partner nobody
;      setshape
;      ask old-partner [ setshape ] ]
;end;;

;; enzyme procedure that controls the rate at which complexed turtles break apart
;to dissociate
;  if partner != nobody
;    [ if ([breed] of partner = substrates) and (random-float 1000 < Kd)
;      [ ask partner [ set partner nobody ]
;        let old-partner partner
;        set partner nobody
;        setshape
;        ask old-partner [ setshape ] ] ]
;end;

;to calculate-velocity
;  let initial-conc substrate-added
;  let current-conc count substrates with [partner = nobody]
;  set v (initial-conc - current-conc) / ticks
;end

;;; plotting procedures

;to plot-concentrations
;  set-current-plot "Concentrations"
;  set-current-plot-pen "Substrate"
;  plot count substrates with [partner = nobody]
;  set-current-plot-pen "Complex"
;  plot count enzymes with [partner != nobody]
;  set-current-plot-pen "Product"
;  plot count products
;end

;to setup-mm-plot
;  set-current-plot "Michaelis-Menten Curve"
;  clear-plot
;end;

;; allows user to plot the concentration versus the velocity on the Michaelis-Menten Curve
;to do-mm-plot
;  set-current-plot "Michaelis-Menten Curve"
;  plotxy substrate-added v
;end


; *** NetLogo 4.0.2 Model Copyright Notice ***
;
; This model was created as part of the projects:
; PARTICIPATORY SIMULATIONS: NETWORK-BASED DESIGN FOR SYSTEMS LEARNING IN
; CLASSROOMS and/or INTEGRATED SIMULATION AND MODELING ENVIRONMENT.
; The project gratefully acknowledges the support of the
; National Science Foundation (REPP & ROLE programs) --
; grant numbers REC #9814682 and REC-0126227.
;
; Copyright 2001 by Uri Wilensky.  All rights reserved.
;
; Permission to use, modify or redistribute this model is hereby granted,
; provided that both of the following requirements are followed:
; a) this copyright notice is included.
; b) this model will not be redistributed for profit without permission
;    from Uri Wilensky.
; Contact Uri Wilensky for appropriate licenses for redistribution for
; profit.
;
; To refer to this model in academic publications, please use:
; Stieff, M. and Wilensky, U. (2001). NetLogo Enzyme Kinetics model.
; http://ccl.northwestern.edu/netlogo/models/EnzymeKinetics.
; Center for Connected Learning and Computer-Based Modeling,
; Northwestern University, Evanston, IL.
;
; In other publications, please use:
; Copyright 2001 Uri Wilensky.  All rights reserved.
; See http://ccl.northwestern.edu/netlogo/models/EnzymeKinetics
; for terms of use.
;
; *** End of NetLogo 4.0.2 Model Copyright Notice ***
@#$#@#$#@
GRAPHICS-WINDOW
637
10
995
369
-1
-1
14.0
1
10
1
1
1
0
1
1
1
-12
12
-12
12
1
1
1
ticks
30.0

BUTTON
4
41
68
74
setup
setup
NIL
1
T
OBSERVER
NIL
NIL
NIL
NIL
1

BUTTON
69
41
130
74
go
go
T
1
T
OBSERVER
NIL
NIL
NIL
NIL
1

SLIDER
136
45
290
78
Max_Questing_Time
Max_Questing_Time
0.0
30
14.0
14
1
NIL
HORIZONTAL

SLIDER
136
10
290
43
Eggs
Eggs
0.0
500.0
250.0
10.0
1
NIL
HORIZONTAL

PLOT
14
226
305
526
Prevalences
Time
Precent Infected
0.0
10.0
0.0
1.0
true
true
"" ""
PENS
"lonestars" 1.0 0 -16777216 true "" ""
"deer" 1.0 0 -10899396 true "" ""
"hosts" 1.0 0 -2674135 true "" ""

MONITOR
14
84
71
129
Ticks
count lonestars
17
1
11

MONITOR
14
128
71
173
Deer
count deer
17
1
11

MONITOR
179
84
274
129
Infected ticks
count lonestars with [disease_status = 1]
17
1
11

MONITOR
14
172
71
217
Hosts
count hosts
17
1
11

MONITOR
179
128
273
173
Infected deer
count deer with [disease_status = 1]
17
1
11

MONITOR
179
173
273
218
Infected hosts
count hosts with [disease_status = 1]
17
1
11

MONITOR
272
173
357
218
Immune hosts
count hosts with [disease_status = 2]
17
1
11

MONITOR
273
129
356
174
Immune deer
count deer with [disease_status = 2]
17
1
11

MONITOR
69
84
179
129
Susceptible ticks
count lonestars with [disease_status = 0]
17
1
11

MONITOR
70
128
179
173
Susceptible deer
count deer with [disease_status = 0]
17
1
11

MONITOR
71
173
179
218
Susceptible hosts
count hosts with [disease_status = 0]
17
1
11

PLOT
311
226
632
526
Populations
Time
Number
0.0
10.0
0.0
10.0
true
true
"" ""
PENS
"lonestars" 1.0 0 -16777216 true "" ""
"deer" 1.0 0 -10899396 true "" ""
"hosts" 1.0 0 -2674135 true "" ""
"ticks_no_egg" 1.0 0 -5825686 true "" ""

SLIDER
352
83
524
116
lonestar_mortality
lonestar_mortality
0
1.0
0.025
0.1
1
NIL
HORIZONTAL

PLOT
650
403
987
647
Ticks_Only
Time
Population
0.0
10.0
0.0
10.0
true
true
"" ""
PENS
"questing" 1.0 0 -16777216 true "" ""
"adult" 1.0 0 -10899396 true "" ""
"nymph" 1.0 0 -13345367 true "" ""
"larvae" 1.0 0 -2674135 true "" ""
"eggs" 1.0 0 -2064490 true "" ""

SLIDER
291
10
465
43
prob_tick_infect_deer
prob_tick_infect_deer
0
1.0
0.0
0.01
1
NIL
HORIZONTAL

SLIDER
423
156
595
189
prob_deer_recover
prob_deer_recover
0
1
0.05
0.01
1
NIL
HORIZONTAL

SLIDER
424
121
596
154
prob_host_recover
prob_host_recover
0
1.0
0.0
0.01
1
NIL
HORIZONTAL

SLIDER
289
43
464
76
prob_deer_infect_tick
prob_deer_infect_tick
0.0
1.0
0.0
0.01
1
NIL
HORIZONTAL

SLIDER
464
10
634
43
prob_tick_infect_host
prob_tick_infect_host
0
1.0
0.0
0.01
1
NIL
HORIZONTAL

SLIDER
464
43
632
76
prob_host_infect_tick
prob_host_infect_tick
0
1.0
0.0
0.01
1
NIL
HORIZONTAL

SLIDER
392
567
564
600
initial_prev
initial_prev
0
100
50.0
1
1
NIL
HORIZONTAL

MONITOR
48
552
108
597
Larvae
count lonestars with [life_stage = 1]
17
1
11

MONITOR
118
552
182
597
Nymphs
count lonestars with [life_stage = 2]
17
1
11

MONITOR
192
553
249
598
Adults
count lonestars with [life_stage = 3]
17
1
11

MONITOR
53
613
110
658
Eggs
count lonestars with [life_stage = 0]
17
1
11

MONITOR
259
623
341
668
NIL
time_check
17
1
11

MONITOR
359
623
433
668
NIL
year_num
17
1
11

SLIDER
461
613
633
646
transovarial_prev
transovarial_prev
0
100
50.0
1
1
NIL
HORIZONTAL

@#$#@#$#@
## WHAT IS IT?

This model demonstrates the kinetics of single-substrate enzyme-catalysis. The interactions between enzymes and substrates are often difficult to understand and the model allows users to visualize the complex reaction.

The standard equation for this reaction is shown below.

                      Kc          Kr
            E + S <=======> E-S ------> E + P
                      Kd

Here E represents Enzyme, S Substrate, E-S Enzyme-Substrate complex, and P product.  The rate constants are Kc for complex formation, Kd for complex dissociation, Kr for catalysis.  The first step in catalysis is the formation of the E-S complex.  This can consist of either covalent or non-covalent bonding.  The rates of complex formation and dissociation are very fast because they are determined by collision and separation of the molecules.  The next step is for the enzyme to catalyze the conversion of substrate to product.  This rate is much slower because the energy required for catalysis is much higher than that required for collision or separation.

The model demonstrates several important properties of enzyme kinetics.  Enzyme catalysis is often assumed to be controlled by the rate of complex formation and dissociation, because it occurs much faster than the rate of catalysis. Thus, the reaction becomes dependent on the ratio of Kc / Kd.  The efficiency of catalysis can be studied by observing catalytic behavior at different substrate concentrations.

By measuring the rate of complex formation at different substrate concentrations, a Michaelis-Menten Curve can be plotted.  Analysis of the plot provides biochemists with the maximum rate (Vmax) at which the reaction can proceed. As can be seen from the model, this plot is linear at low levels of substrate, and non-linear at higher levels of substrate.  By examining the model, the reasons for this relationship can be seen easily.

Enzyme catalysis can also be controlled using inhibitors. Inhibitors are molecules that are structurally similar to substrate molecules that can complex with the enzyme and interfere with the E-S complex formation.  Subsequently, the shape of the Michaelis-Menten Curve will be altered. The model demonstrates the effects of inhibitors on catalysis.

## HOW TO USE IT

Choose the values of Kc, Kd, and Kr with appropriate sliders:  
- Kc controls the rate at which substrates (green) and enzymes (red) stick together so that catalysis can occur  
- Kd controls the rate at which they come unstuck  
- Kr controls the rate of the forward reaction by which an enzyme (red) converts a substrate (green) to a product (blue)

Having chosen appropriate values of the constants, press SETUP to clear the world and create a constant initial number of enzyme (red) molecules. Play with several different values to observe variable effects on complex formation and catalysis.

Press GO to start the simulation.  A constant amount of enzyme (red) will be generated.  The concentrations of substrate, complex, and product are plotted in the CONCENTRATIONS window.

Experiment with using the ADD-SUBSTRATE and ADD-INHIBITOR buttons to observe the effects of adding more molecules to the system manually as it runs.  The default setting for Kr is 0, which means that no product (blue) will be generated unless you change Kr to a non-zero value.

Note that when complexes form they stop moving.  This isn't intended to be physically realistic; it just makes the formation of complexes easier to see.  (This shouldn't affect the overall behavior of the model.)

To plot the Michaelis-Menten Curve for your reaction conditions, you will have to perform several runs at different concentrations in order to measure the velocity for each run. To do this, set the PAUSE? switch ON.  When this switch is on, the model automatically stops after 30 time ticks.  Begin your assay by setting the substrate volume to zero and running the simulation.  When it stops, press RECORD V and a point will be plotted on the Michaelis-Menten Curve. Run another simulation with a higher concentration of substrate by changing the VOLUME slider, then hitting SETUP followed by GO, followed by RECORD V once the model stops.  Continue for several values of substrate concentrations until a curve is generated. If you wish to start over hit CLEAR MM to reset the plot.

## THINGS TO NOTICE

Watch the rate at which the enzyme and substrate stick together. How does this affect the conversion of substrate into product? What would happen if Kd is very high and Kc is very low? If Kr were the same order of magnitude as Kd and Kc?

Watch the Michaelis-Menten Curve. Does it match up with the discussion of enzyme kinetics discussed above? Why does the plot initially slope upward, then flatten out?

Which variables can alter the magnitude of v?

How does the magnitude of Kd and Kr affect the smoothness of the Michaelis-Menten Curve?

## THINGS TO TRY

Run the simulation with VOLUME set to various amounts. How does this affect the curve?

If Kr is MUCH greater than Kd, what affect does this have on the reaction?  How important does complex formation become in this situation?

If Kc is MUCH less than Kd, what does this mean in the real-world? How are the enzyme and substrate related under these conditions?

What effect does adding inhibitor to the model have on the plot? Is Vmax affected?

## EXTENDING THE MODEL

What would happen if yellow inhibitor molecules could react to form a product? How would this affect the plot?

Inhibitors can be irreversible or reversible. That is, they can bind to an enzyme and never let go, or they can stick and fall off. Currently, the model simulates irreversible inhibitors. Modify the code so that the yellow molecules reversibly bind to the enzyme. How does this affect catalysis?

Often, the product of catalysis is an inhibitor of the enzyme. This is called a feedback mechanism. In this model, product cannot complex with enzyme. Modify the procedures so that the product is a reversible inhibitor. How does this affect catalysis with and without yellow inhibitor?

Include a slider that allows you to change the concentration of enzyme.  What affect does this have on the plot?  Vmax?  Look closely!

## NETLOGO FEATURES

It is a little difficult to ensure that a reactant never participates in two reactions simultaneously.  In the future, a primitive called GRAB may be added to NetLogo; then the code in the FORM-COMPLEX procedure wouldn't need to be quite so tricky.

## CREDITS AND REFERENCES

Thanks to Mike Stieff for his work on this model.

To refer to this model in academic publications, please use:  Stieff, M. and Wilensky, U. (2001).  NetLogo Enzyme Kinetics model.  http://ccl.northwestern.edu/netlogo/models/EnzymeKinetics.  Center for Connected Learning and Computer-Based Modeling, Northwestern University, Evanston, IL.

In other publications, please use:  Copyright 2001 Uri Wilensky.  All rights reserved.  See http://ccl.northwestern.edu/netlogo/models/EnzymeKinetics for terms of use.
@#$#@#$#@
default
true
0
Polygon -7500403 true true 150 5 40 250 150 205 260 250

airplane
true
0
Polygon -7500403 true true 150 0 135 15 120 60 120 105 15 165 15 195 120 180 135 240 105 270 120 285 150 270 180 285 210 270 165 240 180 180 285 195 285 165 180 105 180 60 165 15

arrow
true
0
Polygon -7500403 true true 150 0 0 150 105 150 105 293 195 293 195 150 300 150

box
false
0
Polygon -7500403 true true 150 285 285 225 285 75 150 135
Polygon -7500403 true true 150 135 15 75 150 15 285 75
Polygon -7500403 true true 15 75 15 225 150 285 150 135
Line -16777216 false 150 285 150 135
Line -16777216 false 150 135 15 75
Line -16777216 false 150 135 285 75

bug
true
0
Circle -7500403 true true 96 182 108
Circle -7500403 true true 110 127 80
Circle -7500403 true true 110 75 80
Line -7500403 true 150 100 80 30
Line -7500403 true 150 100 220 30

butterfly
true
0
Polygon -7500403 true true 150 165 209 199 225 225 225 255 195 270 165 255 150 240
Polygon -7500403 true true 150 165 89 198 75 225 75 255 105 270 135 255 150 240
Polygon -7500403 true true 139 148 100 105 55 90 25 90 10 105 10 135 25 180 40 195 85 194 139 163
Polygon -7500403 true true 162 150 200 105 245 90 275 90 290 105 290 135 275 180 260 195 215 195 162 165
Polygon -16777216 true false 150 255 135 225 120 150 135 120 150 105 165 120 180 150 165 225
Circle -16777216 true false 135 90 30
Line -16777216 false 150 105 195 60
Line -16777216 false 150 105 105 60

car
false
0
Polygon -7500403 true true 300 180 279 164 261 144 240 135 226 132 213 106 203 84 185 63 159 50 135 50 75 60 0 150 0 165 0 225 300 225 300 180
Circle -16777216 true false 180 180 90
Circle -16777216 true false 30 180 90
Polygon -16777216 true false 162 80 132 78 134 135 209 135 194 105 189 96 180 89
Circle -7500403 true true 47 195 58
Circle -7500403 true true 195 195 58

circle
false
0
Circle -7500403 true true 0 0 300

circle 2
false
0
Circle -7500403 true true 0 0 300
Circle -16777216 true false 30 30 240

complex
true
0
Polygon -2674135 true false 76 47 197 150 76 254 257 255 257 47
Polygon -10899396 true false 79 46 198 148 78 254

cow
false
0
Polygon -7500403 true true 200 193 197 249 179 249 177 196 166 187 140 189 93 191 78 179 72 211 49 209 48 181 37 149 25 120 25 89 45 72 103 84 179 75 198 76 252 64 272 81 293 103 285 121 255 121 242 118 224 167
Polygon -7500403 true true 73 210 86 251 62 249 48 208
Polygon -7500403 true true 25 114 16 195 9 204 23 213 25 200 39 123

cylinder
false
0
Circle -7500403 true true 0 0 300

dot
false
0
Circle -7500403 true true 90 90 120

enzyme
true
0
Polygon -2674135 true false 76 47 197 150 76 254 257 255 257 47

face happy
false
0
Circle -7500403 true true 8 8 285
Circle -16777216 true false 60 75 60
Circle -16777216 true false 180 75 60
Polygon -16777216 true false 150 255 90 239 62 213 47 191 67 179 90 203 109 218 150 225 192 218 210 203 227 181 251 194 236 217 212 240

face neutral
false
0
Circle -7500403 true true 8 7 285
Circle -16777216 true false 60 75 60
Circle -16777216 true false 180 75 60
Rectangle -16777216 true false 60 195 240 225

face sad
false
0
Circle -7500403 true true 8 8 285
Circle -16777216 true false 60 75 60
Circle -16777216 true false 180 75 60
Polygon -16777216 true false 150 168 90 184 62 210 47 232 67 244 90 220 109 205 150 198 192 205 210 220 227 242 251 229 236 206 212 183

fish
false
0
Polygon -1 true false 44 131 21 87 15 86 0 120 15 150 0 180 13 214 20 212 45 166
Polygon -1 true false 135 195 119 235 95 218 76 210 46 204 60 165
Polygon -1 true false 75 45 83 77 71 103 86 114 166 78 135 60
Polygon -7500403 true true 30 136 151 77 226 81 280 119 292 146 292 160 287 170 270 195 195 210 151 212 30 166
Circle -16777216 true false 215 106 30

flag
false
0
Rectangle -7500403 true true 60 15 75 300
Polygon -7500403 true true 90 150 270 90 90 30
Line -7500403 true 75 135 90 135
Line -7500403 true 75 45 90 45

flower
false
0
Polygon -10899396 true false 135 120 165 165 180 210 180 240 150 300 165 300 195 240 195 195 165 135
Circle -7500403 true true 85 132 38
Circle -7500403 true true 130 147 38
Circle -7500403 true true 192 85 38
Circle -7500403 true true 85 40 38
Circle -7500403 true true 177 40 38
Circle -7500403 true true 177 132 38
Circle -7500403 true true 70 85 38
Circle -7500403 true true 130 25 38
Circle -7500403 true true 96 51 108
Circle -16777216 true false 113 68 74
Polygon -10899396 true false 189 233 219 188 249 173 279 188 234 218
Polygon -10899396 true false 180 255 150 210 105 210 75 240 135 240

house
false
0
Rectangle -7500403 true true 45 120 255 285
Rectangle -16777216 true false 120 210 180 285
Polygon -7500403 true true 15 120 150 15 285 120
Line -16777216 false 30 120 270 120

inhib complex
true
0
Polygon -2674135 true false 76 47 197 150 76 254 257 255 257 47
Polygon -1184463 true false 77 48 198 151 78 253 0 253 0 46

inhibitor
true
0
Polygon -1184463 true false 197 151 60 45 1 45 1 255 60 255

leaf
false
0
Polygon -7500403 true true 150 210 135 195 120 210 60 210 30 195 60 180 60 165 15 135 30 120 15 105 40 104 45 90 60 90 90 105 105 120 120 120 105 60 120 60 135 30 150 15 165 30 180 60 195 60 180 120 195 120 210 105 240 90 255 90 263 104 285 105 270 120 285 135 240 165 240 180 270 195 240 210 180 210 165 195
Polygon -7500403 true true 135 195 135 240 120 255 105 255 105 285 135 285 165 240 165 195

line
true
0
Line -7500403 true 150 0 150 300

line half
true
0
Line -7500403 true 150 0 150 150

pentagon
false
0
Polygon -7500403 true true 150 15 15 120 60 285 240 285 285 120

person
false
0
Circle -7500403 true true 110 5 80
Polygon -7500403 true true 105 90 120 195 90 285 105 300 135 300 150 225 165 300 195 300 210 285 180 195 195 90
Rectangle -7500403 true true 127 79 172 94
Polygon -7500403 true true 195 90 240 150 225 180 165 105
Polygon -7500403 true true 105 90 60 150 75 180 135 105

plant
false
0
Rectangle -7500403 true true 135 90 165 300
Polygon -7500403 true true 135 255 90 210 45 195 75 255 135 285
Polygon -7500403 true true 165 255 210 210 255 195 225 255 165 285
Polygon -7500403 true true 135 180 90 135 45 120 75 180 135 210
Polygon -7500403 true true 165 180 165 210 225 180 255 120 210 135
Polygon -7500403 true true 135 105 90 60 45 45 75 105 135 135
Polygon -7500403 true true 165 105 165 135 225 105 255 45 210 60
Polygon -7500403 true true 135 90 120 45 150 15 180 45 165 90

rabbit
false
0
Polygon -7500403 true true 61 150 76 180 91 195 103 214 91 240 76 255 61 270 76 270 106 255 132 209 151 210 181 210 211 240 196 255 181 255 166 247 151 255 166 270 211 270 241 255 240 210 270 225 285 165 256 135 226 105 166 90 91 105
Polygon -7500403 true true 75 164 94 104 70 82 45 89 19 104 4 149 19 164 37 162 59 153
Polygon -7500403 true true 64 98 96 87 138 26 130 15 97 36 54 86
Polygon -7500403 true true 49 89 57 47 78 4 89 20 70 88
Circle -16777216 true false 37 103 16
Line -16777216 false 44 150 104 150
Line -16777216 false 39 158 84 175
Line -16777216 false 29 159 57 195
Polygon -5825686 true false 0 150 15 165 15 150
Polygon -5825686 true false 76 90 97 47 130 32
Line -16777216 false 180 210 165 180
Line -16777216 false 165 180 180 165
Line -16777216 false 180 165 225 165
Line -16777216 false 180 210 210 240

sheep
false
0
Rectangle -16777216 true false 166 225 195 285
Rectangle -16777216 true false 62 225 90 285
Rectangle -7500403 true true 30 75 210 225
Circle -7500403 true true 135 75 150
Circle -16777216 true false 180 76 116

spider
true
0
Polygon -7500403 true true 134 255 104 240 96 210 98 196 114 171 134 150 119 135 119 120 134 105 164 105 179 120 179 135 164 150 185 173 199 195 203 210 194 240 164 255
Line -7500403 true 167 109 170 90
Line -7500403 true 170 91 156 88
Line -7500403 true 130 91 144 88
Line -7500403 true 133 109 130 90
Polygon -7500403 true true 167 117 207 102 216 71 227 27 227 72 212 117 167 132
Polygon -7500403 true true 164 210 158 194 195 195 225 210 195 285 240 210 210 180 164 180
Polygon -7500403 true true 136 210 142 194 105 195 75 210 105 285 60 210 90 180 136 180
Polygon -7500403 true true 133 117 93 102 84 71 73 27 73 72 88 117 133 132
Polygon -7500403 true true 163 140 214 129 234 114 255 74 242 126 216 143 164 152
Polygon -7500403 true true 161 183 203 167 239 180 268 239 249 171 202 153 163 162
Polygon -7500403 true true 137 140 86 129 66 114 45 74 58 126 84 143 136 152
Polygon -7500403 true true 139 183 97 167 61 180 32 239 51 171 98 153 137 162

square
false
0
Rectangle -7500403 true true 30 30 270 270

square 2
false
0
Rectangle -7500403 true true 30 30 270 270
Rectangle -16777216 true false 60 60 240 240

star
false
0
Polygon -7500403 true true 151 1 185 108 298 108 207 175 242 282 151 216 59 282 94 175 3 108 116 108

substrate
true
5
Polygon -10899396 true true 76 47 197 151 75 256

target
false
0
Circle -7500403 true true 0 0 300
Circle -16777216 true false 30 30 240
Circle -7500403 true true 60 60 180
Circle -16777216 true false 90 90 120
Circle -7500403 true true 120 120 60

tree
false
0
Circle -7500403 true true 118 3 94
Rectangle -6459832 true false 120 195 180 300
Circle -7500403 true true 65 21 108
Circle -7500403 true true 116 41 127
Circle -7500403 true true 45 90 120
Circle -7500403 true true 104 74 152

triangle
false
0
Polygon -7500403 true true 150 30 15 255 285 255

triangle 2
false
0
Polygon -7500403 true true 150 30 15 255 285 255
Polygon -16777216 true false 151 99 225 223 75 224

truck
false
0
Rectangle -7500403 true true 4 45 195 187
Polygon -7500403 true true 296 193 296 150 259 134 244 104 208 104 207 194
Rectangle -1 true false 195 60 195 105
Polygon -16777216 true false 238 112 252 141 219 141 218 112
Circle -16777216 true false 234 174 42
Rectangle -7500403 true true 181 185 214 194
Circle -16777216 true false 144 174 42
Circle -16777216 true false 24 174 42
Circle -7500403 false true 24 174 42
Circle -7500403 false true 144 174 42
Circle -7500403 false true 234 174 42

turtle
true
0
Polygon -10899396 true false 215 204 240 233 246 254 228 266 215 252 193 210
Polygon -10899396 true false 195 90 225 75 245 75 260 89 269 108 261 124 240 105 225 105 210 105
Polygon -10899396 true false 105 90 75 75 55 75 40 89 31 108 39 124 60 105 75 105 90 105
Polygon -10899396 true false 132 85 134 64 107 51 108 17 150 2 192 18 192 52 169 65 172 87
Polygon -10899396 true false 85 204 60 233 54 254 72 266 85 252 107 210
Polygon -7500403 true true 119 75 179 75 209 101 224 135 220 225 175 261 128 261 81 224 74 135 88 99

wheel
false
0
Circle -7500403 true true 3 3 294
Circle -16777216 true false 30 30 240
Line -7500403 true 150 285 150 15
Line -7500403 true 15 150 285 150
Circle -7500403 true true 120 120 60
Line -7500403 true 216 40 79 269
Line -7500403 true 40 84 269 221
Line -7500403 true 40 216 269 79
Line -7500403 true 84 40 221 269

x
false
0
Polygon -7500403 true true 270 75 225 30 30 225 75 270
Polygon -7500403 true true 30 75 75 30 270 225 225 270
@#$#@#$#@
NetLogo 6.3.0
@#$#@#$#@
@#$#@#$#@
@#$#@#$#@
<experiments>
  <experiment name="experiment" repetitions="10" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <timeLimit steps="100"/>
    <exitCondition>not any? (lonestars with [disease_status = 1])</exitCondition>
    <metric>count(lonestars with [disease_status = 1])</metric>
    <metric>count(lonestars with [current_activity = 1])</metric>
    <enumeratedValueSet variable="lonestar_mortality">
      <value value="0.025"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="prob_tick_infect_deer">
      <value value="0.1"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="prob_host_infect_tick">
      <value value="0.05"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="prob_deer_infect_tick">
      <value value="0.05"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="prob_host_recover">
      <value value="0"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="Max_Questing_Time">
      <value value="14"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="prob_tick_infect_host">
      <value value="0.05"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="Eggs">
      <value value="250"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="prob_deer_recover">
      <value value="0.05"/>
    </enumeratedValueSet>
  </experiment>
  <experiment name="run_100" repetitions="100" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <timeLimit steps="10000"/>
    <exitCondition>not any? (lonestars with [disease_status = 1])</exitCondition>
    <enumeratedValueSet variable="lonestar_mortality">
      <value value="0.025"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="prob_tick_infect_deer">
      <value value="0.5"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="prob_host_infect_tick">
      <value value="0.25"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="prob_deer_infect_tick">
      <value value="0.25"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="prob_host_recover">
      <value value="0"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="Max_Questing_Time">
      <value value="14"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="prob_tick_infect_host">
      <value value="0.5"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="Eggs">
      <value value="250"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="prob_deer_recover">
      <value value="0"/>
    </enumeratedValueSet>
  </experiment>
</experiments>
@#$#@#$#@
@#$#@#$#@
default
0.0
-0.2 0 0.0 1.0
0.0 1 1.0 0.0
0.2 0 0.0 1.0
link direction
true
0
Line -7500403 true 150 150 90 180
Line -7500403 true 150 150 210 180
@#$#@#$#@
0
@#$#@#$#@
